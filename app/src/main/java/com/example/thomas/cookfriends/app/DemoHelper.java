...
import com.example.thomas.cookfriends.bean.chatbean.EmojiconExampleGroupData;
...
import com.example.thomas.cookfriends.ui.chat.CallReceiver;
import com.example.thomas.cookfriends.ui.chat.ChatActivity;
import com.example.thomas.cookfriends.ui.chat.VideoCallActivity;
import com.example.thomas.cookfriends.ui.chat.VoiceCallActivity;
import com.example.thomas.cookfriends.ui.MainActivity;
import com.example.thomas.cookfriends.utils.PreferenceManager;
import com.example.thomas.cookfriends.utils.UserProfileManager;
...
public class DemoHelper {
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactAdded(String username) {
            ...
            CookUser aUser = new CookUser(user.getUsername());
            if (!localUsers.containsKey(username)) {
                userDao.saveContact(aUser);
            }
	...
	private EaseUser getUserInfo(String username){
        ...
        query.findObjects(new FindListener<CookUser>() {
            ...
            public void done(List<CookUser> list, BmobException e) {
                if(list == null)
                    return;
                for(CookUser aUser : list){
                    user.setAvatar(aUser.getAvatar());
                    user.setNickname(aUser.getNick());
                }
    ...
    public void saveContact(CookUser user){
        ...
        easeUser.setNickname(user.getNick());
        easeUser.setAvatar(user.getAvatar());
    	contactList.put(user.getUsername(), easeUser);
    ...
    public void updateContactList(List<EaseUser> contactInfoList) {
         ...
         ArrayList<CookUser> mList = new ArrayList<CookUser>();
         for(EaseUser easeUser : contactList.values()){
             CookUser aUser = new CookUser(easeUser.getUsername());
             aUser.setNick(easeUser.getNick());
             aUser.setNick(easeUser.getNickname());
             aUser.setAvatar(easeUser.getAvatar());
             mList.add(aUser);
         }
           ...
           public void run(){
               ...
               try {
                  ...
                  if (selfIds.size() > 0) {
                   ...
                   List<CookUser> users = new ArrayList<CookUser>();
                   for(EaseUser easeUser : userlist.values()){
                       CookUser aUser = new CookUser(easeUser.getUsername());
                       aUser.setAvatar(easeUser.getAvatar());
                       aUser.setNick(easeUser.getNickname());
                       users.add(aUser);
